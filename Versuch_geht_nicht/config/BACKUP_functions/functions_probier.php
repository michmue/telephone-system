...
function func_call_type($source, $destination)
    {
        ...
        if($len_source == 2 && $len_destination == 2)
          ...
          elseif(($len_source > 2 || $len_source == 1 || empty($source)) && ($len_destination == 2 || $destination == 89004921435907075678 || in_array($destination, $amt)))
            {
...
function func_call_numbers($sting)
	...
	# Telefonnummern des Anrufes ermitteln
        ...
        # Nummer 1: 0 bzw. 66 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[66], $tmp_array);
        if ($tmp_array[0][0] == "0" || $tmp_array[0][0] == "0")
            {
                $call_number[0] = substr($call_number[0], 1);
            }

        # Nummer 1: 0 bzw. 60 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[60], $tmp_array);
        if ($tmp_array[0][0] == "0" || $tmp_array[0][0] == "0")
            {
                $call_number[0] = substr($call_number[0], 1);
            }
        
        # Nummer 1: 0 bzw. 61 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[61], $tmp_array);
        if ($tmp_array[0][0] == "0" || $tmp_array[0][0] == "0")
            {
                $call_number[0] = substr($call_number[0], 1);
            }
        ...
        # Nummer 2: 0 bzw 66 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[1], $tmp_array);
        if ($tmp_array[0][0] == "66" || $tmp_array[0][0] == "0")
            {
                $call_number[1] = substr($call_number[1], 1);
            }
     
        # Nummer 2: 0 bzw 60 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[1], $tmp_array);
        if ($tmp_array[0][0] == "60" || $tmp_array[0][0] == "0")
            {
                $call_number[1] = substr($call_number[1], 1);
            }

        # Nummer 2: 0 bzw 61 am Anfang entfernen
        preg_match_all('/([0-9])/', $call_number[1], $tmp_array);
        if ($tmp_array[0][0] == "61" || $tmp_array[0][0] == "0")
            {
                $call_number[1] = substr($call_number[1], 1);
            }
