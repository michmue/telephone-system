...
if (!$use_archiv)
	...
	while(!feof($fp))
		...
		if ($new_call)
			...
			if ($count_in_new_call[$akt_call_number] == 2)
				...
				if (!empty($call_dest_tmp))
					...
					# Ermitteln ob über das Interface TEL1 geroutet wird (Dieser eintrag wird ausgeblendet!!)
					$call_dest_tmp_3 				= split(":", $call_dest_tmp[1]);
					$call_dest_gw[$akt_call_number]	= ltrim($call_dest_tmp_3[0]);
		...
		# Anruf ausgeben
		if ($write_call && $call_dest_gw[$close_call] != "TEL1")
			{				
			# Counter erhöhen
			$counter++;
			$tmp_dest_suche = "";
			...
			$dauer 							= $uhrzeit_ende[$close_call] - $uhrzeit_beginn[$close_call];
			...
			if ((!empty($call_int[$close_call]) && !empty($call_dest[$close_call])) || (!empty($call_sor[$close_call]) && !empty($call_amt[$close_call])))
				...
				$tabelle[$counter]['dauer']			= date("H:i:s", $dauer - 3600);
				...
				if ($call_owner[$close_call] == $_owner || $_owner == 0)
					...
					$tabelle_a[$counter]['dauer']		= date("H:i:s", $dauer - 3600);
